...
export function TakeOrder() {
  ...
  // Menu items
  const menuItems = [
    'Burger', 'Wings', 'Fries', 'Onion Rings', 'Jalapeño Poppers', 
    'Artichoke Dip', 'Brownie Sundae', 'Tiramisu', 'Soft Drink', 'Beer',
  ];

  // Read orders from localStorage when the component loads
  useEffect(() => {
    const savedOrders = JSON.parse(localStorage.getItem('orders'));
    if (savedOrders) {
      setOrderMap(new Map(savedOrders));
    }
  }, []);

  // Save orders to localStorage whenever the OrderMap state changes
  useEffect(() => {
    if (OrderMap.size > 0) {
      localStorage.setItem('orders', JSON.stringify(Array.from(OrderMap.entries())));
    }
  }, [OrderMap]);
